...
inline auto convert(const ceph::buffer::list& from) -> std::vector<std::uint8_t>
{
 ...
 std::vector<std::uint8_t> out_data(sizeof(std::uint32_t) + from.size()); 
 zpp::bits::out out(out_data);

 auto ptr = const_cast<ceph::buffer::list&>(from).c_str();

 // Until I find a nicer way to get a contiguous view (trying to use the iterator
 // interface in buffer::list led to nothing but trouble!), I subtract one to avoid
 // copying the NULL that will be appended: 
